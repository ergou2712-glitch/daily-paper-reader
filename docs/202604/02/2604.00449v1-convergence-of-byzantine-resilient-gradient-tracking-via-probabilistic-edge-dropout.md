...
result: "理论上证明了算法在不同隔离条件下的线性收敛性，实验中在MNIST数据集上比坐标修剪均值法准确率提升高达4.3%。"
conclusion: 通过概率边缘丢弃和漏积分技术，该研究为去中心化网络提供了具备强鲁棒性和收敛保障的梯度追踪方案。
---

## 摘要
我们研究了包含可能发送任意对抗性消息的拜占庭智能体的网络分布式优化问题。我们提出了带概率边丢弃的梯度追踪（GT-PD），这是一种随机梯度追踪方法，能够在对抗性通信下保持梯度追踪的收敛特性。GT-PD 结合了两个互补的防御层：一种通用的以自我为中心的投影，将每个传入消息裁剪到接收智能体周围半径为 $τ$ 的球内；以及一种完全去中心化的概率丢弃规则，由决策和追踪通道中的双指标信任评分驱动。该设计在限制对抗性扰动的同时，保留了双随机混合结构，而这一性质在去中心化设置下的鲁棒聚合中通常会丢失。在完全拜占庭隔离（$p_b=0$）的情况下，GT-PD 线性收敛至仅由随机梯度方差决定的邻域。对于部分隔离（$p_b>0$），我们引入了带概率边丢弃和漏积分的梯度追踪（GT-PD-L），它利用漏积分器控制由持续扰动引起的追踪误差累积，并实现线性收敛至由随机方差和裁剪-泄漏比决定的有界邻域。我们进一步证明，在 $p_h=1$ 的两层丢弃下，隔离拜占庭智能体不会为诚实节点的共识动态引入额外方差。在 MNIST 上针对符号翻转、ALIE 和内积操纵攻击的实验表明，在隐蔽攻击下，GT-PD-L 的性能优于逐坐标修剪均值高达 4.3 个百分点。
