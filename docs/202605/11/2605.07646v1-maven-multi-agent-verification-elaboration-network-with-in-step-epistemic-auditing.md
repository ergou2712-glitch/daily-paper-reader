...
score: 7.0
evidence: 高风险应用中信任的认识论审计
tldr: 针对大语言模型在复杂推理中容易出现错误累积且缺乏中间验证的问题，本文提出了MAVEN框架。该框架借鉴黑板架构，通过“怀疑者-研究者-裁判”多智能体协作，将逻辑防御与事实依据分离，实现了步进式的认知审计。实验表明，MAVEN在多个基准测试中优于Gemini-3.1-Pro等模型，能生成显式、模块化且可验证的推理轨迹，是一种通用的推理增强方案。
selection_source: fresh_fetch
motivation: 现有的单链条推理模式缺乏中间步骤的验证机制，导致早期错误会级联放大，且难以进行细粒度的认知审计。
method: 构建了一个基于黑板架构的多智能体网络，通过“怀疑者-研究者-裁判”的对抗性循环来解耦逻辑验证与事实支撑。
result: 在OpenBookQA、TruthfulQA等多个数据集上，MAVEN的推理质量显著超过了Gemini-3.1-Pro和ReConcile等基准模型。
conclusion: MAVEN作为一种模型无关的推理增强器，通过显式的审议轨迹显著提升了不同基座模型的推理性能和认知可信度。
---

## 摘要
尽管显式推理轨迹增强了模型的可解释性，但现有范式通常依赖于缺乏中间验证的单体链，导致早期错误在不受控的情况下发生级联。这种模块化的缺失阻碍了细粒度的审计，并损害了高风险应用所需的认识论信任。我们提出了 MAVEN（具有步内认识论审计的多智能体验证-阐述网络），这是一个受黑板模型启发的框架，旨在通过显式的角色解耦将大语言模型（LLM）转变为审慎的推理者。MAVEN 的核心是运行一个对抗性的“怀疑者-研究者-裁判”循环，通过在功能上分离逻辑辩护与事实依据，来模拟专家的研讨过程。在 OpenBookQA、TruthfulQA、HALUEVAL 和 StrategyQA 基准测试上的实验表明，MAVEN 在四个细粒度指标上均表现出卓越的推理质量。值得注意的是，MAVEN 通过生成显式结构化、模块化且可验证的研讨轨迹，而非依赖隐式内部状态或事后共识，其表现持续优于 GEMINI-3.1-Pro 等潜在推理模型以及基于共识的基准模型（如 ReConcile）。此外，综合评估证实 MAVEN 完全与模型无关，可作为一种强大且可迁移的推理增强器，在多种骨干模型上均能显著提升性能。
